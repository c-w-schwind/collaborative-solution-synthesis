...
const translateEntityNumberToId = (entityType, paramName) => async (req, res, next) => {
    try {
        const entityNumber = parseInt(req.params[paramName], 10);
        ...
        req.params.id = entity._id.toString();
        next();
    } catch (error) {
        next(error);
    }
